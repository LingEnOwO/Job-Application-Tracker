...
import { createApplication } from "../../services/storage.js";
...
function populateForm(data) {
  ...
  if (data.applyDate)
    document.getElementById("applyDate").value = data.applyDate;
  if (data.stage) document.getElementById("stage").value = data.stage;
  if (data.jobId) document.getElementById("jobId").value = data.jobId;
  if (data.resumeVersion)
    document.getElementById("resumeVersion").value = data.resumeVersion;
  if (data.referral)
    document.getElementById("referral").checked = data.referral;
  ...
  try {
    ...
    if (response.success) {
      ...
      if (response.data.atsDetected) {
        showStatus(
          `Detected ${response.data.atsDetected} ATS. Please review extracted data.`,
          "info",
        );
      } else {
    ...
    showStatus(
      "Could not auto-extract data. Please fill in manually.",
      "error",
    );
  }
}
...
async function saveApplication(event) {
  ...
  try {
    ...
    if (!hasAnyData) {
      throw new Error(
        "Please fill in at least one field (URL, company, position, etc.)",
      );
    }
